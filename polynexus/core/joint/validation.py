...
def validate_triple_phi_c(phi_dsc, phi_waxs, phi_saxs, tolerance=0.05, sample_id="", weights=None):
    ...
    if len(available) < 2:
        results.append(CrossValidationResult(
            ...
            status="SKIP", passed=True, severity="INFO",
...
def validate_tm_bidirectional(tm_dsc, L_saxs, lc_saxs, polymer_family="", sigma_e=0.093, delta_Hf=None, rho_c=1.0, tolerance=3.0, sample_id=""):
    ...
    if tm_dsc is None or np.isnan(tm_dsc):
        results.append(CrossValidationResult(check_name=f"{pfx}Tm_GT",status="SKIP",passed=True,severity="INFO",message="Tm check skipped: no DSC Tm"))
        return results
    if lc_saxs is None or np.isnan(lc_saxs) or lc_saxs<=0:
        results.append(CrossValidationResult(check_name=f"{pfx}Tm_GT",status="SKIP",passed=True,severity="INFO",message="Tm check skipped: no SAXS lc"))
        return results
    if delta_Hf is None and polymer_family:
        delta_Hf = _DHM0.get(polymer_family.upper())
    if delta_Hf is None or delta_Hf<=0:
        results.append(CrossValidationResult(check_name=f"{pfx}Tm_GT",status="SKIP",passed=True,severity="INFO",message="Tm check skipped: no delta_Hf"))
        return results
    tm_inf = _TM_INF.get(polymer_family.upper(), 300.0)
    dH_vol = delta_Hf*rho_c*1e6
    lc_m = lc_saxs*1e-9
    tm_gt = tm_inf*(1-2*sigma_e/(dH_vol*lc_m))
    diff=abs(tm_dsc-tm_gt)
    passed=diff<=tolerance
    results.append(CrossValidationResult(
        check_name=f"{pfx}Tm_GT",status="OK" if passed else "DIFF",passed=passed,severity="OK" if passed else "ERROR",
...
def validate_L_consistency(L_bragg, L_corr, tolerance=0.03, sample_id=""):
    ...
    if L_bragg is None or np.isnan(L_bragg):
        return CrossValidationResult(check_name=f"{pfx}L_consistency",status="SKIP",passed=True,severity="INFO",message="L check skipped: no Bragg L")
    if L_corr is None or np.isnan(L_corr):
        return CrossValidationResult(check_name=f"{pfx}L_consistency",status="SKIP",passed=True,severity="INFO",message="L check skipped: no corr L")
    diff=abs(L_bragg-L_corr)
    rel=diff/max(L_bragg,L_corr,1e-9)
    passed=rel<=tolerance
    return CrossValidationResult(check_name=f"{pfx}L_consistency",status="OK" if passed else "DIFF",passed=passed,severity="OK" if passed else "WARN",message=f"L(Bragg)={L_bragg:.2f} vs L(corr)={L_corr:.2f} nm diff={diff:.2f} ({rel*100:.1f}%)",details={"L_bragg_nm":L_bragg,"L_corr_nm":L_corr,"diff_nm":diff,"rel_diff_pct":rel*100})

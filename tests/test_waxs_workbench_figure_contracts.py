...
def test_waxs_profiles_use_real_static_temperature_and_strain_manifest_ids() -> None:
    assert tuple(link.key for link in profile_for("waxs.static").figure_links) == (
        "waxs.static.profile",
        "waxs.static.fit.si",
    )
    assert tuple(link.key for link in profile_for("waxs.temperature").figure_links) == (
        "waxs.temperature.evolution",
        "waxs.temperature.full-series.si",
    )
    assert tuple(link.key for link in profile_for("waxs.strain").figure_links) == (

...
from tests.test_joint_lifecycle_closure import _rows as _joint_rows
...
def test_native_windows_gui_real_route_capture(
    ...
    _assert_package_export(editor, app, mode)
    print(f"NATIVE_GUI_ROUTE {mode} run={run_id} captures={capture_root}")


def test_native_windows_gui_joint_synthetic_route(tmp_path: Path) -> None:
    output_root = tmp_path / "joint-output"
    report = JointCoordinator().publish_hub_report(
        _joint_rows(), output_root, run_id="native-joint-route"
    )
    entries = build_active_manifest_gallery_entries(output_root)
    assert entries
    ...
    assert window._restore_history_record(
        {
            ...
            "results_summary": {
                "result": report,
                "figure_run_id": "native-joint-route",
            },
        }
    )
    window._populate_plots()
    ...
    for index, surface in ((2, "results"), (3, "gallery"), (4, "history")):
        window._tabs.setCurrentIndex(index)
        app.processEvents()
        assert window.grab().save(str(capture_root / f"joint_compare_{surface}.png"))

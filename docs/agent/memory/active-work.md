...
- Mainline integration reconciliation is active on isolated branch `codex/mainline-integration-reconciliation` as of 2026-07-12. Baseline inventory, shared figure lifecycle compatibility, SAXS mode/evidence contract, cumulative review evidence, and completed temperature/strain evidence filtering are recorded in `docs/acceptance/2026-07-12-mainline-integration-reconciliation-inventory.md`; review findings were fixed in `a81af58`, temperature filtering is in `985f9c2`/`a4231f8`, and strain filtering is in `c75888b`. Human scientific review of the SAXS slices completed with no blocking concerns. Current limitations: `scripts/verify.py` is user-local and absent from clean `origin/main`, full pytest previously exceeded the runtime limit, and external PR/merge review is pending. No push, PR, merge, or local-main sync has been performed.

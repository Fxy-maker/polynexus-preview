...
- DSC publication packs are implemented on `codex/dsc-publication-packs-v2` from merged main `2fca4e81`. The slice contains only standard/isothermal/non-isothermal DSC evidence-gated providers, the DSC publication asset profile, shared panel-label serialization, and compatibility coverage; local DSC/shared regressions and the branch quality gate pass. Human DSC scientific/publication review, CI, push, and merge remain pending.

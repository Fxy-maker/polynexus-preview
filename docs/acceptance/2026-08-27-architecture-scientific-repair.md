...
- Template identity is revalidated from the serialized canonical template,
  including its content hash, template ID, conversion hash, source artifact,
  and declared technique.
- Completed projections must also carry dataset, plan, and result records with
  artifact/technique linkage; minimal forged ComputeRun objects are rejected.

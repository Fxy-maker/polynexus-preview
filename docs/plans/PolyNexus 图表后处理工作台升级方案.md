...
- 支持基础画布操作：选择、拖动、删除、撤销、重做、裁剪、适配窗口、100% 显示。
- 支持保存当前文件和另存为新文件。
...
自动验收已覆盖编辑器核心等价流程；发布前仍建议人工打开 GUI 走查一次按钮手感和视觉反馈：
- 对一张 PNG 进入编辑器。
- 添加文本、箭头、矩形，并框选裁剪一次画布。
- 保存到当前图，确认文件变化且生成 `.bak`。
- 重新打开编辑器，确认 annotation state 可读取且裁剪尺寸保持。
- 另存为副本，确认原图未被覆盖，且新副本有自己的 `asset_spec`。
- 主窗口保存回调已有自动测试覆盖：静态图保存不触发自动重绘，另存为会把新图加入图库并选中。

## 6. 验收标准

第一阶段完成后，必须满足：

- 用户能直接对当前成品图添加文字、箭头、矩形、高亮，并裁剪画布。
- 点击保存后，当前图文件立即变化，无需再点“重新绘图”。
- 裁剪后保存会输出裁剪后的真实图片尺寸，并同步更新 `asset_spec` 宽高。

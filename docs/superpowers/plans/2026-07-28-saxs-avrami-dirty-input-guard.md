# SAXS Avrami dirty-input guard Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans (recommended). Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Reuse the existing numeric coercion policy at the public Avrami
time/Xc boundary without changing the kinetics model or scientific gates.

**Architecture:** Coerce both arrays into detached float arrays, align the
common prefix, and pass them through the current finite/positive-time mask,
Xc range selection, and fit body. No new missing-data policy is introduced.
...
**Files:**
- Create: `tests/test_saxs_avrami_dirty_input.py`

- [ ] Add malformed-token, finite/non-finite, mismatched-length, clean
...
- [ ] Coerce and align local time/Xc arrays before the current valid mask.
- [ ] Leave the fit body, range selection, point gates, and result dictionary
  unchanged.

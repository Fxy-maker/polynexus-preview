...
NMR_ROOT = PROJECT_ROOT / "\u6d4b\u8bd5\u6570\u636e" / "NMR"
OUTPUT_DIR = PROJECT_ROOT / "results" / "nmr_rag_warmup"
LOG_DIR = OUTPUT_DIR / "logs"
SUMMARY_PATH = OUTPUT_DIR / "summary.json"

SUPPORTED_EXTS = {".csv", ".txt", ".jdx", ".dx", ".nmr", ".jdf", ".bin"}
...
def main() -> int:
    ...
    files = _find_nmr_files()
    print(f"found_nmr_files={len(files)} root={NMR_ROOT}", flush=True)
...
def _find_nmr_files() -> list[Path]:
    files: list[Path] = []
    for path in sorted(NMR_ROOT.rglob("*")):
        if not path.is_file():
            continue
        if "polynexus_output" in str(path):
            continue
        suffix = path.suffix.lower()
        if suffix in SUPPORTED_EXTS or path.name.lower() == "fid":
            files.append(path)
    return files
...
def _safe_name(name: str) -> str:
    return re.sub(r"[^A-Za-z0-9_.()\-]+", "_", name).strip("_") or "nmr_case"


def _format_table(rows: list[dict]) -> str:
    lines = ["file | baseline_r2 | best_r2 | delta_r2 | converged | remark", "--- | ---: | ---: | ---: | --- | ---"]
    for row in rows:
        lines.append(
            f"{row['file']} | {_fmt(row.get('baseline_r_squared'))} | {_fmt(row.get('best_r_squared'))} | "
            f"{_fmt(row.get('delta_r_squared'))} | {row.get('converged')} | {row.get('remark')}"
        )
    return "\n".join(lines)

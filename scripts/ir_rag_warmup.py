...
IR_ROOT = PROJECT_ROOT / "测试数据" / "IR"
OUTPUT_DIR = PROJECT_ROOT / "results" / "ir_rag_warmup"
...
def main() -> int:
    ...
    files = [
        path
        for path in sorted(IR_ROOT.rglob("*"))
        if path.is_file()
        and path.suffix.lower() in {".csv", ".spa"}
        and "polynexus_output" not in str(path)
        and "_results" not in str(path)
    ]
    print(f"found_ir_files={len(files)} root={IR_ROOT}", flush=True)
    ...
    for index, data_file in enumerate(files, start=1):
        ...
        print(
            ...
            f"converged={row.get('converged')} remark={row.get('remark')}",
            flush=True,
        )
...
def _safe_name(name: str) -> str:
    return re.sub(r"[^A-Za-z0-9_.()\\-]+", "_", name).strip("_") or "ir_case"
